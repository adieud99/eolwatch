...
현재 시연 구성은 **로컬 관리 VM 1대 + 대상 VM 2대**다. 아래에서 1절(로컬 개발 실행)과 5절(SSH 점검 계정)은 현재도 그대로 적용한다. **2~4절의 AWS Terraform·DNS·운영 Compose 절차는 과거 실증 기록이며 현재 시연의 전제 조건이 아니다.** 실제 생성 결과는 [AWS 실제 배포 기록](AWS_DEPLOYMENT_RECORD.md), 관리 VM 코드 갱신은 [작업 재개](NEXT_SESSION.md#코드-작업-후-검증배포)를 따른다.
