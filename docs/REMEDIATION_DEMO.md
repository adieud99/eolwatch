...
EOLWatch는 운영자가 브라우저에서 서버와 소스의 취약점을 검사하고 조치 근거를 비교하는 개발 및 인프라 취약점 검사 솔루션이다. Syft가 설치 구성요소를 수집해 SBOM을 만들고 Grype가 CVE를 연결한다. EOLWatch는 서버 등록, 검사 실행, 이력 보관, 전후 비교와 운영자의 조치 상태 기록을 담당한다. 이 문서는 인프라 검사 축의 시연이다. Black Duck 계정은 필요하지 않다.
...
운영자는 `인프라 검사`의 서버 행에서 검사 범위를 선택한다.
...
- **관리 앱:** EOLWatch 웹의 `인프라 검사`(서버 목록·검사 작업)와 `검사 기록`(CVE 결과·전후 비교) 화면.
...
2. `http://127.0.0.1:18080`에서 로그인 → `인프라 검사` → `LAB-VM-01` 행에서 검사 범위 `데모 앱 · Python` → `취약점 검사`를 누른다. 먼저 `SSH 점검`을 눌러 서버 정보를 수집해 두면 함께 보여줄 수 있다.
...
6. `검사 기록` → `검사 이력`에서 3번의 이전 검사와 5번의 새 검사를 선택해 `검사 전후 비교`를 연다. 버전 변화, 계속 검출, 새로 검출, 재분석 미검출, 구성요소 제거를 확인한다.
