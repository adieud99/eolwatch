...
**2026-09-15~16 시점의 검증 기록이다.** 당시 메뉴 이름(작업 대상·프로젝트 분석·SBOM 원장·CVE 조치·제품·계약)으로 수행했다. 제품 수명주기·계약·고객사·사이트·소프트웨어·EOL 관련 항목은 2026-09-18 재범위화에서 제거한 기능의 당시 기록이며 현재 기능이 아니다. 검사 이력 번호와 ZIP·백업 증거는 지금도 유효하다.
...
- 자산 편집, 제품 공통 수명주기와 영향 자산, 계약 등록·수정. 개별 구성요소 날짜 우선 적용 및 제품 날짜 상속. (제품·계약·수명주기는 2026-09-18 제거)
...
- 고객사·사이트·자산·소프트웨어·사용자 등록 후 폼 초기화와 목록 갱신 오류 수정. (고객사·사이트·소프트웨어 등록은 2026-09-18 제거)
- 서버 시간대와 무관하게 설정한 업무 시간대(기본 Asia/Seoul)로 EOL(당시)·조치 기한 초과·목록 기준일을 계산. 자정 경계 검사 포함.
...
## 관리 화면과 SBOM 상세 (당시 기록)

이 절의 제품 날짜·계약·구성요소 개별 날짜 기능은 2026-09-18에 제거했다. SBOM 상세(의존성 목록)와 CVE 조치 보존은 현재도 유효하다.
