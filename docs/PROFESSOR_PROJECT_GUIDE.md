...
| 축 | 사용자 | 솔루션 | 결과 |
|---|---|---|---|
| 개발 검사 | 소스 ZIP 업로드 | Syft → SPDX 2.3 → Grype (+OSV) | 라이브러리별 CVE·수정 버전·심각도 |
...
- 이전 검사 원본과 담당자 기록을 보존해 "무엇을 보고 조치했는지"를 다시 확인할 수 있다.
...
| 검사 기록 · 검사 이력 | 서버·범위·날짜로 과거 검사 검색, 저장 ZIP 재검사, 비교 선택 | 재현 가능한 이력 |
| 검사 기록 · CVE 결과·조치 | 탐지 결과·수정 버전·조치 관리 | 탐지 후 누가 무엇을 처리하는가 |
| 검사 기록 · 조치 작업목록 | 담당자·기한·미지정·상태별 업무 처리 | 전체 이력의 미완료 조치 |
| 검사 기록 · 의존성 목록 | 구성요소·버전·라이선스·의존관계·원본 | 검사 결과의 구성 근거 |
| 검사 기록 · 검사 전후 비교 | 이전·이후 검사 비교, PDF·JSON | 같은 서버·범위의 변화 |
| 관리 | 사용자 권한, 감사 로그 | 변경 추적 |
...
| 순서 | 보여줄 화면/행동 | 설명할 내용 |
|---|---|---|
| 1 | `개발 검사`에서 서버·프로젝트 이름·ZIP 선택 | 결과를 묶는 기준과 입력 제한(50 MiB, 명세·잠금 파일) |
...
**상황:** 운영 서버에 설치된 라이브러리에서 CVE가 검출됐다. 담당자가 버전을 올리고 서비스가 정상인지 확인한 뒤 검사 근거와 함께 조치를 기록한다.
...
| 4 | `검사 기록` → `CVE 결과·조치` → `조치 관리` | 담당자·기한·검토 메모를 기록 |
...
2. 저장된 ZIP의 파일명·크기·SHA-256을 확인하고 **저장된 ZIP 재검사**를 실행해 재현성을 보여준다.
3. 이전·이후 검사를 선택해 `검사 전후 비교`로 이동하고 PDF·JSON을 내려받는다.
4. `조치 작업목록`에서 담당자 미지정·기한 초과·심각도 필터를 적용하고, 특정 조치를 기록한 뒤 새로고침해도 담당자·기한·이력이 보존되는지 보여준다.
5. `개요`에서 전체 이력의 미조치 CVE와 현재 기준 미조치 CVE를 구분한다. 과거 기록을 지우지 않고 현재 위험을 계산하는 이유를 설명한다.

## 7. 운영·추가 서비스

- **권한·감사:** `관리`에서 관리자/조회자 계정과 감사 로그를 보여준다. 조회자로 로그인하면 검사 요청·조치 버튼이 비활성화된다.
- **정기 검사(추가 서비스):** `POST /api/analyses/schedules`로 서버·범위·주기를 등록하면 worker가 기한마다 검사를 큐에 넣는다. 예약은 DB에 남는다. 현재 화면 메뉴에는 없으므로 API 문서로 소개한다.
...
| 서버에 에이전트를 설치해야 하는가? | 아니다. 요청 때 SSH로 Syft를 복사해 실행하고 읽기 전용 명령으로 서버 정보를 수집한다. |
...
| 아무 프로그램이나 첨부 가능한가? | 제한된 소스 ZIP과 등록 서버를 지원하며 포함된 패키지 자료를 읽는다. 컨테이너 이미지·Git URL은 지원하지 않는다. |
