...
검사 결과: 백엔드 **348 passed / 2 skipped**, 프런트엔드 **100 passed**, 프런트엔드 빌드 성공 (2026-09-21).
...
1. **개발·인프라 두 흐름 브라우저 시연 검증.** 개발: ZIP 업로드 → 결과 → 검사 기록. 인프라: 서버 등록 → SSH 점검·취약점 검사 → 서버 정보 카드 → 결과 → 검사 기록. 이전에 남아 있던 ZIP 재검사·실행 중 취소·과거 비교 검증도 여기서 끝낸다.
