...
| 영역 | 현재 MVP | 목표 아키텍처 |
|---|---|---|
| 자산 | 고객·사이트·제품·계약 API 구현 | 역할 권한과 감사 로그 추가 |
| SBOM | 공식 스키마 검증·정규화·영향 탐색 구현 | 버전 비교와 VEX 추가 |
| 수집 | SSH 수집·이력·일일 worker 구현 | 실제 EC2 3대 실증, AWS SSM 추가 |
| 취약점 | 미구현 | purl 기반 OSV 조회와 VEX 상태 |
| 인증 | 미구현 | 관리자·조회자 2단계 RBAC |
| 알림 | 미구현 | Teams Workflows와 전송 이력 |
| 배포 | 개발·운영 Compose와 Terraform 검증 | 실제 도메인 AWS 배포 |
