"""History beyond 100 rows, source dedup and project summaries over real DB/API paths."""
...
def test_all_250_results_page_without_raw_json(client, factory, asset):
    with factory() as db:
    ...
    try:
        ...
        assert client.get('/api/analyses/runs/999').status_code == 404
...
def test_same_source_is_deduplicated_and_rescanned_after_completion(client, factory, settings, asset):
    ...
    assert first['upload_sha256'] == hashlib.sha256(source_zip()).hexdigest()
    with factory() as db:
        assert db.scalar(select(func.count(models.AnalysisUpload.id))) == 1
    complete_job(factory, first['id'], status='SUCCESS')
    again = put_source(client, asset)
    assert again.status_code == 202, again.text
    assert again.json()['id'] != first['id']
    assert again.json()['upload_id'] == first['upload_id']


def test_busy_asset_rejects_a_second_project_until_the_scan_finishes(client, factory, asset):
    first = put_source(client, asset).json()
    blocked = put_source(client, asset, name='next-project', payload=source_zip(content=b'Jinja2==3.1.6\n'))
    assert blocked.status_code == 409
    assert '진행 중' in blocked.json()['detail']
    with factory() as db:
        assert db.scalar(select(func.count(models.AnalysisJob.id))) == 1
    complete_job(factory, first['id'])
    queued = put_source(client, asset, name='next-project', payload=source_zip(content=b'Jinja2==3.1.6\n'))
    assert queued.status_code == 202, queued.text
    assert queued.json()['scan_scope'] == 'source-zip:next-project'
...
def test_project_groups_separate_assets_scopes_versions_and_use_latest_time(client, factory, asset):
    ...
    assert row['analysis_count'] == 2 and row['job_count'] == 2
    assert row['latest_analysis']['id'] == 1
    assert client.get('/api/analyses/projects', params={'asset_id': other}).json()['total'] == 1


def test_viewer_reads_history_but_cannot_upload(client, factory, asset):
    first = put_source(client, asset).json()
    complete_job(factory, first['id'])
    ...
    assert client.get('/api/analyses/history').status_code == 200
    assert client.get(f"/api/analyses/jobs/{first['id']}").status_code == 200
    assert put_source(client, asset).status_code == 403
