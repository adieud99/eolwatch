...
export default function AssetEditor({ asset, canEdit, request, onSaved, onClose, onViewCve, onViewSbom }) {
  ...
  useEffect(() => {
    ...
    setReady(false); setLoading(true); setPending(false); setError(''); setDeleteOpen(false); setConfirmation(''); setPurge(false); setPassword(''); setPrivateKey(''); setResetHostKey(false)
  ...
  async function save(event) {
    ...
    if (privateKey.trim() && cleaned.ssh_auth === 'private_key') values.ssh_private_key = privateKey.trim()
    ...
    if (cleaned.ssh_auth === 'private_key' && !privateKey.trim() && !current.has_private_key) { setError('키 파일을 첨부하세요.'); return }
  ...
  return <section ref={panel} className="asset-editor panel" aria-label="서버 정보 관리">
    ...
    {detailTab === 'overview' && !canEdit && <dl className="asset-editor-read"><dt>서버 이름</dt><dd>{current.name}</dd><dt>유형</dt><dd>{types[current.asset_type] || current.asset_type}</dd>{textFields.map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{current[key] || '미입력'}</dd></div>)}<dt>SSH 포트</dt><dd>{current.ssh_port}</dd><dt>인증 방식</dt><dd>{current.ssh_auth === 'password' ? '비밀번호' : current.ssh_auth === 'private_key' ? '키 파일' : '관리 서버 키 (구 방식)'}</dd><dt>호스트 키</dt><dd>{current.ssh_host_key_fingerprint || '아직 접속 전'}</dd><dt>검사 대상</dt><dd>{current.monitored ? '사용' : '해제'}</dd></dl>}
    ...
    {canEdit && !deleteOpen && <form onSubmit={save}>
      <fieldset disabled={!ready || pending} className="asset-editor-grid">
        ...
        <label>{draft.ssh_auth === 'private_key' ? '키 암호 (있을 때만)' : 'SSH 비밀번호'}<input type="password" autoComplete="new-password" disabled={draft.ssh_auth === 'key'} value={password} placeholder={draft.ssh_auth === 'key' ? '관리 서버 키 사용 시 불필요' : current.has_password ? '저장된 값 있음 · 바꾸려면 입력' : draft.ssh_auth === 'private_key' ? '키에 암호가 없으면 비움' : '접속 비밀번호'} onChange={(e) => { setPassword(e.target.value); setError('') }} /></label>
        <label>인증 방식<select value={draft.ssh_auth} onChange={(e) => change('ssh_auth', e.target.value)}><option value="password">비밀번호로 접속 (키 없이)</option><option value="private_key">키 파일 첨부</option>{current.ssh_auth === 'key' && <option value="key">관리 서버 키 (구 방식)</option>}</select></label>
        {draft.ssh_auth === 'private_key' && <label className="asset-editor-wide">SSH 개인키 파일 (id_ed25519, id_rsa, .pem 등){current.has_private_key && !privateKey && <small className="subtle">저장된 키 파일 있음 · 바꾸려면 새 파일 선택</small>}<input type="file" onChange={async (e) => { const file = e.target.files?.[0]; setError(''); try { setPrivateKey(file ? (await readTextFile(file)).trim() : '') } catch (reason) { setError(reason.message) } }} /></label>}
        {draft.ssh_auth !== 'key' && <div className="asset-editor-wide"><span className="subtle">호스트 키: {current.ssh_host_key_fingerprint || '아직 접속 전 · 처음 접속 때 기억합니다'}</span>{current.ssh_host_key_fingerprint && <label className="asset-editor-check"><input type="checkbox" checked={resetHostKey} onChange={(e) => setResetHostKey(e.target.checked)} />호스트 키 초기화 (서버를 재설치했을 때)</label>}</div>}
