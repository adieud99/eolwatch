...
function ComponentDetail({ component, sbomId, request, onSelect, onClose }) {
  ...
  return <section className="workspace-detail" aria-label="구성요소 상세">
    <div className="workspace-title"><div><h3>{component.name} {component.version}</h3><p>분석 당시 구성요소 · {component.supplier || '공급자 미기록'}</p></div><button className="secondary" onClick={onClose}>상세 닫기</button></div>
...
export default function SbomExplorer({ sbomId, request, download, onClose, onViewCve }) {
  ...
  return <section className="panel workspace-panel" aria-label="SBOM 상세 탐색">
    ...
    {busy ? <p role="status">구성요소를 불러오는 중…</p> : <div className="table-wrap"><table aria-label="SBOM 구성요소"><thead><tr><th>패키지</th><th>분석 당시 버전</th><th>공급자</th><th>라이선스</th><th>상세</th></tr></thead><tbody>{page.items.map((item) => <tr key={item.id}><td><strong>{item.name}</strong><small>{item.purl || item.bom_ref}</small></td><td>{item.version || '미기록'}</td><td>{item.supplier || '미기록'}</td><td>{item.licenses?.map(display).join(', ') || '미기록'}</td><td><button className="table-button" aria-label={`${item.name} ${item.version || ''} 상세 보기`} onClick={() => choose(item.id)}>상세·관계</button></td></tr>)}{!page.items.length && !error && <tr><td colSpan={5} className="empty">검색 조건에 맞는 구성요소가 없습니다.</td></tr>}</tbody></table></div>}
