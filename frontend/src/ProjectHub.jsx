...
function RunButtons({ run, onViewResult, onViewSbom, onDownload, busy }) {
  return <div className="action-row"><button className="table-button" onClick={() => onViewResult(run.sbom_id)}>CVE 결과</button><button className="table-button" onClick={() => onViewSbom(run.sbom_id)}>SBOM 구성요소</button><button className="table-button" disabled={busy} onClick={() => onDownload(`/analyses/${run.id}/bundle`, `eolwatch-analysis-${run.id}.json`, `bundle-${run.id}`)}>원본 번들</button></div>
}
...
function ProjectDetail({ group: initialGroup, canEdit, request, download, onJobQueued, onChanged, onViewResult, onViewSbom, onCompare, onNewAnalysis, onViewWork }) {
  ...
  function startNew() {
    ...
    onNewAnalysis({ assetId: group.asset_id, scanScope: group.scan_scope, projectName: group.project_name || (group.scan_scope.startsWith('source-zip:') ? group.scan_scope.slice(separator + 1) : ''), targetPath: group.scan_scope.startsWith('ssh-') && separator >= 0 ? group.scan_scope.slice(separator + 1) : '' })
  }

  return <section className="panel project-detail" aria-label="선택한 프로젝트">
    <div className="project-heading"><div><span className="eyebrow">PROJECT HISTORY</span><h2>{group.asset_tag} · {scopeLabel(group.scan_scope)}</h2><p>{group.asset_name} · 같은 자산과 분석 범위의 모든 이력을 조회합니다.</p></div><div className="action-row">{onViewWork && <button className="secondary" onClick={() => onViewWork(group.asset_id)}>이 자산 조치목록</button>}{canEdit && isExecutableScope(group.scan_scope) && <button className="primary" onClick={startNew}>이 프로젝트 새 분석</button>}</div></div>
    <ReadState state={metadata} name="선택한 프로젝트 정보" />
    <div className="project-summary"><div><strong>마지막 성공 분석</strong><p>{group.latest_analysis ? `분석 #${group.latest_analysis.id} · CVE ${group.latest_analysis.cve_count}건` : '성공한 분석 없음'}</p><small>{dateTime(group.latest_analysis?.imported_at)}</small></div><div><strong>마지막 작업</strong><p>{group.latest_job ? `#${group.latest_job.id} · ${jobLabels[group.latest_job.status] || group.latest_job.status}` : '작업 없음'}</p><small>{dateTime(group.latest_job?.requested_at)}</small>{group.latest_job?.error_message && <small>{group.latest_job.error_message}</small>}</div><div><strong>정기 분석</strong><p>{group.schedule ? group.schedule.enabled ? `${group.schedule.interval_minutes}분마다 실행` : '일시 중지' : '예약 없음'}</p><small>{group.schedule?.enabled ? `다음 실행 ${dateTime(group.schedule.next_run_at)}` : '서버 앱 분석 화면에서 관리할 수 있습니다.'}</small></div></div>
    <p className="project-help">CVE 수는 각 분석 당시 원본 보고서 기준입니다. 이후 작업이 실패하거나 취소되어도 마지막 성공 결과의 분석 시점은 바뀌지 않습니다.</p>
    <div className="project-tabs" role="tablist" aria-label="프로젝트 이력 종류">{[['results', '분석 결과'], ['jobs', '작업 이력'], ['uploads', '저장된 ZIP']].map(([value, label]) => <button className="secondary" key={value} role="tab" aria-selected={tab === value} onClick={() => switchTab(value)}>{label}</button>)}</div>
    {tab !== 'uploads' && <form className="project-filters" onSubmit={(event) => { event.preventDefault(); if (draft.date_from && draft.date_to && draft.date_from > draft.date_to) { setError('조회 시작일은 종료일보다 늦을 수 없습니다.'); return } setError(''); setFilters({ ...draft }); setOffset(0) }}>
      <label>이력 검색<input value={draft.q} onChange={(event) => setDraft({ ...draft, q: event.target.value })} placeholder="자산 이름, 분석 범위" /></label><label>조회 시작일<input type="date" value={draft.date_from} onChange={(event) => setDraft({ ...draft, date_from: event.target.value })} /></label><label>조회 종료일<input type="date" value={draft.date_to} onChange={(event) => setDraft({ ...draft, date_to: event.target.value })} /></label>{tab === 'jobs' && <label>작업 상태<select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value })}><option value="ALL">전체 상태</option>{Object.entries(jobLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>}<button className="primary">이력 조회</button>
    </form>}
    {error && <p className="form-error" role="alert">{error}</p>}{message && <p role="status" className="project-help">{message}</p>}
    <ReadState state={page} name={tab === 'results' ? '분석 결과' : tab === 'jobs' ? '작업 이력' : '저장된 ZIP'} />
    {tab === 'results' && <>
      <div className="project-comparison" aria-label="분석 전후 선택"><div>이전: {base ? `분석 #${base.id}` : '선택 안 함'} · 이후: {target ? `분석 #${target.id}` : '선택 안 함'}<small>서로 다른 페이지의 분석도 선택할 수 있습니다. 이후 분석은 이전 분석보다 나중에 수집된 결과여야 합니다.</small></div><button className="primary" disabled={!comparablePair || !!pending} onClick={() => runOperation('compare', (signal) => onCompare(base.id, target.id, signal), () => {})}>{pending === 'compare' ? '비교 준비 중…' : '선택한 분석 비교'}</button>{(base || target) && <button className="secondary" onClick={() => { setBase(null); setTarget(null) }}>비교 선택 해제</button>}{base && target && !comparablePair && <p className="form-error">같은 자산·범위에서 시간 순서에 맞는 서로 다른 분석을 선택하세요.</p>}</div>
      <div className="table-wrap"><table aria-label="프로젝트 분석 결과"><thead><tr><th>분석·시각</th><th>도구·구성요소</th><th>CVE</th><th>결과·원본</th><th>전후 비교</th></tr></thead><tbody>{items.map((run) => <tr key={run.id}><td><strong>분석 #{run.id}</strong><small>{dateTime(run.imported_at)}</small><small>SBOM #{run.sbom_id}</small></td><td>{run.scanner} {run.scanner_version}<small>구성요소 {run.component_count}개</small></td><td>{run.cve_count}건</td><td><RunButtons run={run} onViewResult={onViewResult} onViewSbom={onViewSbom} onDownload={downloadFile} busy={!!pending} /></td><td><div className="action-row"><button className="table-button" aria-pressed={base?.id === run.id} onClick={() => setBase(run)}>이전으로 선택</button><button className="table-button" aria-pressed={target?.id === run.id} onClick={() => setTarget(run)}>이후로 선택</button></div></td></tr>)}</tbody></table></div>
    </>}
    {tab === 'jobs' && <div className="table-wrap"><table aria-label="프로젝트 작업 이력"><thead><tr><th>작업·입력</th><th>상태</th><th>요청·종료</th><th>결과·관리</th></tr></thead><tbody>{items.map((job) => <tr key={job.id}><td><strong>작업 #{job.id}</strong><small>{job.upload_filename || job.target_path || scopeLabel(job.scan_scope)}</small>{job.retry_of_id && <small>작업 #{job.retry_of_id} 재시도</small>}</td><td>{jobLabels[job.status] || job.status}{job.error_message && <small>{job.error_code}: {job.error_message}</small>}</td><td>{dateTime(job.requested_at)}<small>종료: {dateTime(job.finished_at)}</small></td><td><div className="action-row">{job.status === 'SUCCESS' && job.sbom_id && <><button className="table-button" onClick={() => onViewResult(job.sbom_id)}>CVE 결과</button><button className="table-button" onClick={() => onViewSbom(job.sbom_id)}>SBOM 구성요소</button></>}{canEdit && job.status === 'FAILED' && <button className="table-button" disabled={!!pending} onClick={() => retry(job)}>{pending === `retry-${job.id}` ? '재시도 요청 중…' : '실패 작업 재시도'}</button>}{canEdit && (cancellableStatuses.has(job.status) || job.status === 'CANCEL_REQUESTED') && <button className="table-button" disabled={!!pending || job.status === 'CANCEL_REQUESTED'} onClick={() => cancel(job)}>{pending === `cancel-${job.id}` ? '취소 요청 중…' : job.status === 'CANCEL_REQUESTED' ? '취소 처리 중' : '분석 취소'}</button>}{job.status === 'IMPORTING' && <small>결과 저장이 먼저 끝나면 취소되지 않을 수 있습니다.</small>}</div></td></tr>)}</tbody></table></div>}
    {tab === 'uploads' && <><p className="project-help">보관된 ZIP은 새 작업의 입력으로 다시 사용할 수 있습니다. 크기·보관 상태를 표시하며 다운로드·재분석 시 SHA-256을 검증합니다.</p><div className="table-wrap"><table aria-label="프로젝트 저장된 ZIP"><thead><tr><th>파일·등록 시각</th><th>원본 식별정보</th><th>분석 이력</th><th>관리</th></tr></thead><tbody>{items.map((upload) => <tr key={upload.id}><td><strong>{upload.filename}</strong><small>{dateTime(upload.created_at)}</small><small>{bytes(upload.size_bytes)} · {storageLabels[upload.storage_status] || upload.storage_status}</small></td><td><code>{upload.sha256}</code><small>업로드 #{upload.id}</small></td><td>{upload.job_count}회 요청<small>{upload.latest_job ? `최근 #${upload.latest_job.id} · ${jobLabels[upload.latest_job.status] || upload.latest_job.status}` : '요청 없음'}</small>{upload.latest_job?.sbom_id && <button className="table-button" onClick={() => onViewResult(upload.latest_job.sbom_id)}>최근 CVE 결과</button>}</td><td><div className="action-row"><button className="table-button" disabled={!!pending || upload.storage_status !== 'AVAILABLE'} onClick={() => downloadFile(`/analyses/uploads/${upload.id}/raw`, upload.filename, `raw-${upload.id}`)}>ZIP 다운로드</button>{canEdit ? <button className="table-button" disabled={!!pending || upload.storage_status !== 'AVAILABLE'} onClick={() => reanalyse(upload)}>{pending === `upload-${upload.id}` ? '재분석 요청 중…' : '저장된 ZIP 재분석'}</button> : <small>재분석은 관리자 권한 필요</small>}</div></td></tr>)}</tbody></table></div></>}
    {!page.loading && !page.error && !items.length && <p className="project-empty">{tab === 'uploads' ? '이 프로젝트에 저장된 ZIP이 없습니다.' : '조건에 맞는 이력이 없습니다. 검색어나 날짜 범위를 조정하세요.'}</p>}
    ...
    <button className="secondary" disabled={page.loading} onClick={page.refresh}>현재 이력 새로고침</button>
  </section>
}
...
export default function ProjectHub({ assets, canEdit, request, download, onChanged, onJobQueued, onViewResult, onViewSbom, onCompare, onNewAnalysis, initialAssetId, initialScope, onSelectionChange, onViewWork }) {
  ...
  useEffect(() => {
    ...
    setSelected(initialScope ? { asset_id: Number(initialAssetId), scan_scope: initialScope, asset_tag: asset?.asset_tag || `자산 #${initialAssetId}`, asset_name: asset?.name || '' } : null)
