...
function ProjectDetail({ group: initialGroup, canEdit, request, download, onJobQueued, onChanged, onViewResult, onViewSbom, onCompare, onNewAnalysis, onViewCve, onTargetDeleted }) {
  ...
  return <section className="panel project-detail" aria-label="선택한 프로젝트">
    ...
    <div className="project-summary"><div><strong>마지막 성공 검사</strong><p>{group.latest_analysis ? `검사 #${group.latest_analysis.id} · CVE ${group.latest_analysis.cve_count}건` : '성공한 검사 없음'}</p><small>{dateTime(group.latest_analysis?.imported_at)}</small></div><div><strong>마지막 작업</strong><p>{group.latest_job ? `#${group.latest_job.id} · ${jobLabels[group.latest_job.status] || group.latest_job.status}` : '작업 없음'}</p><small>{dateTime(group.latest_job?.requested_at)}</small>{group.latest_job?.error_message && <small>{group.latest_job.error_message}</small>}</div></div>
    <p className="project-help">CVE 수는 각 검사 당시 보고서 기준입니다. 이후 작업이 실패하거나 취소되어도 마지막 결과는 그대로입니다.</p>
    <div className="project-tabs" role="tablist" aria-label="이력 종류">{[['results', '검사 결과'], ['jobs', '작업 이력']].map(([value, label]) => <button className="secondary" key={value} role="tab" aria-selected={tab === value} onClick={() => switchTab(value)}>{label}</button>)}</div>
    <form className="project-filters" onSubmit={(event) => { event.preventDefault(); if (draft.date_from && draft.date_to && draft.date_from > draft.date_to) { setError('조회 시작일은 종료일보다 늦을 수 없습니다.'); return } setError(''); setFilters({ ...draft }); setOffset(0) }}>
    ...
    <ReadState state={page} name={tab === 'results' ? '검사 결과' : '작업 이력'} />
    {tab === 'results' && <>
    ...
    {!page.loading && !page.error && !items.length && <p className="project-empty">조건에 맞는 이력이 없습니다.</p>}
    {page.data && <PageControls name={tab === 'results' ? '검사' : '작업'} total={page.data.total} offset={offset} busy={page.loading} onPage={setOffset} />}
    <button className="secondary" disabled={page.loading} onClick={page.refresh}>새로고침</button>
  </section>
}

export default function ProjectHub({ assets, canEdit, request, download, onChanged, onJobQueued, onViewResult, onViewSbom, onCompare, onNewAnalysis, initialAssetId, initialScope, onSelectionChange, onViewCve, onTargetDeleted }) {
  ...
  return <div className="project-hub">
    <section className="panel"><div className="project-heading"><div><span className="eyebrow">PROJECTS & HISTORY</span><h2>프로젝트별 검사 이력</h2><p>대상과 검사 범위별로 마지막 결과와 전체 이력을 봅니다.</p></div>{canEdit && <button className="primary" onClick={() => onNewAnalysis({ assetId: filters.asset_id ? Number(filters.asset_id) : undefined })}>새 검사</button>}</div>
      ...
      <div className="table-wrap"><table aria-label="프로젝트 목록"><thead><tr><th>대상 · 검사 범위</th><th>마지막 결과</th><th>마지막 작업</th><th>이력</th><th>보기</th></tr></thead><tbody>{items.map((item) => <tr key={groupKey(item)} className={group && groupKey(group) === groupKey(item) ? 'project-selected' : ''}><td><strong>{item.asset_tag} · {item.asset_name}</strong><small>{scopeLabel(item.scan_scope)}</small></td><td>{item.latest_analysis ? <>CVE {item.latest_analysis.cve_count}건<small>검사 #{item.latest_analysis.id} · {dateTime(item.latest_analysis.imported_at)}</small></> : '성공한 검사 없음'}</td><td>{item.latest_job ? <>{jobLabels[item.latest_job.status] || item.latest_job.status}<small>작업 #{item.latest_job.id} · {dateTime(item.latest_job.requested_at)}</small></> : '작업 없음'}</td><td>검사 {item.analysis_count} · 작업 {item.job_count} · ZIP {item.upload_count}<small>{item.schedule ? item.schedule.enabled ? `예약 ${item.schedule.interval_minutes}분` : '예약 일시 중지' : '예약 없음'}</small></td><td><button className="table-button" aria-label={`${item.asset_tag} ${scopeLabel(item.scan_scope)} 이력 보기`} onClick={() => { setSelected(item); onSelectionChange?.({ assetId: item.asset_id, scanScope: item.scan_scope }) }}>이력 보기</button></td></tr>)}</tbody></table></div>
      {!groups.loading && !groups.error && !items.length && <p className="project-empty">조건에 맞는 프로젝트가 없습니다.{canEdit ? ' 개발 검사에서 소스를 올리거나 인프라 검사에서 서버를 검사하세요.' : '관리자가 검사를 실행하면 여기서 결과를 볼 수 있습니다.'}</p>}
      {groups.data && <PageControls name="프로젝트" total={groups.data.total} offset={offset} busy={groups.loading} onPage={setOffset} />}
    </section>
    {group ? <ProjectDetail key={groupKey(group)} group={group} canEdit={canEdit} request={request} download={download} onChanged={() => { groups.refresh(); onChanged?.() }} onJobQueued={(job) => { groups.refresh(); onJobQueued(job) }} onViewResult={onViewResult} onViewSbom={onViewSbom} onCompare={onCompare} onNewAnalysis={onNewAnalysis} onViewCve={onViewCve} onTargetDeleted={(id) => { setSelected(null); onSelectionChange?.({}); groups.refresh(); onChanged?.('대상과 이력을 삭제했습니다.'); onTargetDeleted?.(id) }} /> : <p className="project-empty">프로젝트의 ‘이력 보기’를 누르면 결과와 작업 이력이 나옵니다.</p>}
  </div>
}
