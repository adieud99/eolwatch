...
async function selectZip() { fireEvent.click(await screen.findByRole('button', { name: 'APP-1 소스 ZIP · orders 이력 보기' })); await screen.findByText('분석 #204') }
const resultRow = (id) => screen.getByText(`분석 #${id}`, { selector: 'strong' }).closest('tr')
...
describe('프로젝트별 서버 페이지 이력', () => {
  it('205개 분석 중 마지막 페이지를 직접 조회하고 오래된 결과·SBOM·원본 및 전후 비교로 이동한다', async () => {
    ...
    fireEvent.click(within(resultRow(205)).getByRole('button', { name: '이후로 선택' }))
    fireEvent.change(screen.getByLabelText('분석 이동할 쪽'), { target: { value: '11' } })
    fireEvent.click(screen.getByRole('button', { name: '분석 이동' }))
    ...
    fireEvent.click(within(resultRow(1)).getByRole('button', { name: '이전으로 선택' }))
    await act(async () => fireEvent.click(screen.getByRole('button', { name: '선택한 분석 비교' })))
  ...
  it('프로젝트 검색과 날짜·상태를 서버 쿼리로 전달하고 잘못된 기간은 요청하지 않는다', async () => {
    ...
    fireEvent.change(screen.getByLabelText('프로젝트 자산'), { target: { value: '2' } })
  ...
  it('미래 작업 실패와 0 CVE 성공 분석을 구분하고 다른 범위 선택 시 비교를 초기화한다', async () => {
    ...
    expect(within(detail).getByText('분석 #205 · CVE 0건')).toBeInTheDocument()
    expect(within(detail).getByText('#70 · 실패')).toBeInTheDocument()
    fireEvent.click(within(resultRow(205)).getByRole('button', { name: '이전으로 선택' }))
    fireEvent.click(within(resultRow(204)).getByRole('button', { name: '이후로 선택' }))
    expect(screen.getByRole('button', { name: '선택한 분석 비교' })).toBeDisabled()
    expect(screen.getByText(/시간 순서에 맞는 서로 다른 분석/)).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'APP-1 Ubuntu 설치 패키지 이력 보기' }))
    await screen.findByRole('heading', { name: 'APP-1 · Ubuntu 설치 패키지' })
  ...
  it('ZIP 원본 식별정보를 보여주고 다운로드 실패 후 재시도·중복 없는 재분석을 지원한다', async () => {
    let complete
    ...
    const replay = screen.getByRole('button', { name: '저장된 ZIP 재분석' })
  ...
  it('보관 파일이 없으면 다운로드·재분석을 막고 조회자는 모든 편집 기능이 없다', async () => {
    ...
    expect(screen.getByRole('button', { name: '저장된 ZIP 재분석' })).toBeDisabled()
    view.unmount()
    open({ canEdit: false })
    await selectZip(); fireEvent.click(screen.getByRole('tab', { name: '저장된 ZIP' }))
    await screen.findByText('재분석은 관리자 권한 필요')
    expect(screen.getByRole('button', { name: 'ZIP 다운로드' })).toBeEnabled()
    expect(screen.queryByRole('button', { name: /새.*분석|ZIP 재분석|보관 현황/ })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('tab', { name: '작업 이력' }))
    await screen.findByText('작업 #70', { selector: 'strong' })
  ...
  it('취소 요청은 한 번만 보내고 취소 처리 중 상태가 끝날 때까지 중복 취소를 막는다', async () => {
    ...
    const cancel = await screen.findByRole('button', { name: '분석 취소' })
  ...
  it('최근 목록 밖의 실패 작업을 원래 입력으로 재시도하고 오류 후에도 다시 요청할 수 있다', async () => {
    let attempts = 0
    const request = vi.fn(async (path, options) => { if (options?.method === 'POST') { if (++attempts === 1) throw new Error('다른 분석이 실행 중입니다.'); return { ...job, id: 207, status: 'QUEUED', retry_of_id: 70 } } return route(path) })
    const { props } = open({ request }); await selectZip(); fireEvent.click(screen.getByRole('tab', { name: '작업 이력' }))
    fireEvent.click(await screen.findByRole('button', { name: '실패 작업 재시도' }))
    await screen.findByText('다른 분석이 실행 중입니다.')
    await act(async () => fireEvent.click(screen.getByRole('button', { name: '실패 작업 재시도' })))
    const posts = request.mock.calls.filter(([, options]) => options?.method === 'POST')
  ...
  it('첫 프로젝트 페이지 밖의 범위도 정확한 메타데이터를 읽고 문맥 저장 후 상세를 유지한다', async () => {
    ...
    fireEvent.click(within(resultRow(204)).getByRole('button', { name: '이전으로 선택' }))
    view.rerender(<ProjectHub {...view.props} initialAssetId={1} initialScope="source-zip:orders" />)
    expect(screen.getByText('이전: 분석 #204 · 이후: 선택 안 함')).toBeInTheDocument()
    expect(within(screen.getByRole('region', { name: '선택한 프로젝트' })).getByText('분석 #205 · CVE 0건')).toBeInTheDocument()
  })
  ...
  it('프로젝트 전환으로 이전 읽기·다운로드를 중단하고 늦은 응답을 버린다', async () => {
    ...
    expect(screen.queryByText('분석 #204')).not.toBeInTheDocument()
    fireEvent.click(within(resultRow(300)).getByRole('button', { name: '원본 번들' }))
  ...
  it('다른 탭에서 받은 프로젝트 범위를 열고 새 분석 입력에 동일 자산·이름을 보낸다', async () => {
  ...
  it('수동 가져오기 범위의 이력은 읽을 수 있지만 기록되지 않은 입력으로 재실행하지 않는다', async () => {
    ...
    expect(screen.queryByRole('button', { name: '이 프로젝트 새 분석' })).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: '새 프로젝트 분석' })).toBeEnabled()
