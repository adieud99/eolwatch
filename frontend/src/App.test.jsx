...
function cvePage(findings, url) {
  const params = new URL(url, 'http://localhost').searchParams
  const rows = workPage(findings, url.replace('/cves?', '?').replace(/&limit=\d+/, '&limit=1000000').replace(/&offset=\d+/, '')).items
  const rank = { CRITICAL: 5, HIGH: 4, MEDIUM: 3, LOW: 2, NONE: 1 }
  const groups = new Map()
  rows.forEach((finding) => {
    const group = groups.get(finding.cve_id) || { cve_id: finding.cve_id, summary: finding.summary, severity: 'UNKNOWN', link_count: 0, open_count: 0, update_available_count: 0, no_update_count: 0, fixed_versions: [], links: [] }
    group.link_count += 1; group.links.push(finding)
    if (finding.fix_check === 'UPDATE_AVAILABLE') group.update_available_count += 1
    if (finding.fix_check === 'NO_UPDATE_FOUND') group.no_update_count += 1
    if (['AFFECTED', 'UNDER_INVESTIGATION'].includes(finding.vex_status)) group.open_count += 1
    if ((rank[finding.severity] || 0) > (rank[group.severity] || 0)) group.severity = finding.severity
    ...
    groups.set(finding.cve_id, group)
  })
  const items = [...groups.values()].sort((a, b) => (rank[b.severity] || 0) - (rank[a.severity] || 0) || a.cve_id.localeCompare(b.cve_id))
  const limit = Number(params.get('limit') || 25)
  const offset = Number(params.get('offset') || 0)
  return { items: items.slice(offset, offset + limit), total: items.length, limit, offset, as_of: '2026-09-15' }
}
...
describe('웹에서 서버 취약점 분석 실행', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (url, options) => {
      ...
      if (url.startsWith('/api/vulnerability-work/cves?')) return response(cvePage(data.findings || [], url))
  ...
  it('대시보드는 누적 이력과 OS·앱의 마지막 성공 검사을 구분하고 해당 결과로 이동한다', async () => {
    const latest = { asset_id: 7, asset_tag: 'VM-001', asset_name: '실습 서버', scan_scope: 'ubuntu-dpkg-installed', analysis_run_id: 90, sbom_id: 9, cve_count: 0, last_success_at: '2026-09-15T05:00:00Z' }
    data['/api/dashboard/summary'] = { ...data['/api/dashboard/summary'], open_cves: 3219, affected_assets: 1, latest_analyses: [latest, { ...latest, asset_id: 8, asset_tag: 'VM-002', asset_name: '운영 서버', analysis_run_id: 80, sbom_id: 8, cve_count: 3216 }] }
    ...
    const appRow = within(table.getByText('VM-001', { selector: 'strong' }).closest('tr'))
    const osRow = within(table.getByText('VM-002', { selector: 'strong' }).closest('tr'))
    ...
    await act(async () => { fireEvent.click(appRow.getByRole('button', { name: 'VM-001 OS 설치 패키지 최근 검사 결과 보기' })) })
    expect(screen.getByLabelText('확인할 SBOM')).toHaveValue('9')
    expect(fetch.mock.calls.every(([, options]) => !options.method)).toBe(true)
  })
  ...
  it('대시보드에서 진행·실패 상태를 갱신하면서 이전 성공 시점과 CVE 수를 유지한다', async () => {
    ...
    await openApp()
    data['/api/analyses/jobs'] = [{ ...queued, scan_scope: 'ubuntu-dpkg-installed', status: 'SCANNING' }]
    await act(async () => { await vi.advanceTimersByTimeAsync(3000) })
    const table = within(screen.getByRole('table', { name: '대상 · 범위별 최근 검사' }))
    expect(table.getByText('재검사 진행 중입니다. 마지막 성공 검사를 표시합니다.')).toBeInTheDocument()
    data['/api/analyses/jobs'] = [{ ...failed, scan_scope: 'ubuntu-dpkg-installed' }, { ...failed, id: 51, asset_id: 8, asset_tag: 'VM-002', scan_scope: 'ubuntu-dpkg-installed' }]
    await act(async () => { await vi.advanceTimersByTimeAsync(3000) })
    ...
    data['/api/analyses/jobs'] = [{ ...queued, scan_scope: 'ubuntu-dpkg-installed', status: 'CANCELLED' }]
    await act(async () => { await vi.advanceTimersByTimeAsync(3000) })
  ...
  it('다른 사용자의 새 분석 완료를 보면 요약만 한 번 갱신해 대시보드에 반영한다', async () => {
    const latest = { asset_id: 7, asset_tag: 'VM-001', asset_name: '실습 서버', scan_scope: 'ubuntu-dpkg-installed', analysis_run_id: 80, sbom_id: 8, cve_count: 3, last_success_at: '2026-09-15T05:00:00Z' }
    data['/api/dashboard/summary'].latest_analyses = [latest]
    data['/api/analyses/jobs'] = [{ ...queued, scan_scope: 'ubuntu-dpkg-installed' }]
    await openApp('VIEWER')
    ...
    data['/api/analyses/jobs'] = [{ ...queued, scan_scope: 'ubuntu-dpkg-installed', status: 'SUCCESS', sbom_id: 9, analysis_run_id: 90 }]
    await act(async () => { await vi.advanceTimersByTimeAsync(3000) })
    const table = within(screen.getByRole('table', { name: '대상 · 범위별 최근 검사' }))
    expect(table.getByText('검사 #90 · SBOM #9')).toBeInTheDocument()
    expect(table.getByText('0개')).toBeInTheDocument()
...
describe('SBOM 취약점 분석 흐름', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (url, options) => {
      if (url.startsWith('/api/vulnerability-work/cves?')) return jsonResponse(cvePage(data.findings, url))
  ...
  it('CVE 행은 조치 상태를 표시하고 바로 상태를 변경하지 않는다', async () => {
  ...
  it('조치 패널에서 상태와 메모를 저장한 뒤 패널을 닫고 CVE 목록을 갱신한다', async () => {
    await openSecurity()
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: '검사 20 CVE 보기' })) })
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'CVE별로 보기' })) })
    ...
    expect(JSON.parse(post[1].body)).toEqual({ expected_revision: 0, status: 'UNDER_INVESTIGATION', detail: '영향 범위 확인 및 업데이트 일정 검토', justification: null, response: null })
    ...
    expect(fetch.mock.calls.filter(([url]) => url === '/api/vulnerability-work/cves?sbom_id=2&status=OPEN&fix=ALL&kernel=false&relevance=actionable&limit=100&offset=0')).toHaveLength(2)
  })
  ...
  it('다른 CVE를 선택하면 이전 조치 초안을 새 항목에 넘기지 않는다', async () => {
    ...
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: '검사 20 CVE 보기' })) })
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'CVE별로 보기' })) })
    fireEvent.click(screen.getByRole('button', { name: `${newFinding.cve_id} 패키지 2개 펼치기` }))
    fireEvent.click(screen.getByRole('button', { name: `${newFinding.cve_id} current-package 조치 관리` }))
  ...
  it('같은 CVE가 여러 패키지에 붙으면 CVE별 보기에서 한 줄로 접고 펼치면 패키지별 조치가 나온다', async () => {
    data.findings = [newFinding, { ...newFinding, link_id: 21, component_name: 'second-package', vex_status: 'UNDER_INVESTIGATION', fixed_versions: ['3.5'] }, { ...newFinding, link_id: 22, component_name: 'third-package', vex_status: 'FIXED' }]
    await openSecurity()
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: '검사 20 CVE 보기' })) })
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'CVE별로 보기' })) })
    const findings = within(screen.getByRole('table', { name: '선택한 SBOM의 CVE' }))
    expect(findings.getAllByText(newFinding.cve_id)).toHaveLength(1)
    expect(findings.getByText('패키지 2개')).toBeInTheDocument()   // the FIXED link is hidden by the default OPEN filter
    expect(findings.getByText('current-package, second-package')).toBeInTheDocument()
    expect(findings.getByLabelText(`${newFinding.cve_id} 조치 상태`)).toHaveTextContent('미조치 2 / 2')
    expect(findings.getByText('2.1, 3.0, 3.5')).toBeInTheDocument()
    expect(screen.getByText('CVE 1개 · 1–1개 표시')).toBeInTheDocument()
    expect(findings.queryByLabelText(`${newFinding.cve_id} second-package 조치 상태`)).not.toBeInTheDocument()
    fireEvent.click(findings.getByRole('button', { name: `${newFinding.cve_id} 패키지 2개 펼치기` }))
    const packages = within(screen.getByRole('table', { name: `${newFinding.cve_id} 패키지별 조치` }))
    expect(packages.getByLabelText(`${newFinding.cve_id} second-package 조치 상태`)).toHaveTextContent('조사 중')
    expect(packages.getByRole('button', { name: `${newFinding.cve_id} current-package 조치 관리` })).toBeEnabled()
    fireEvent.click(findings.getByRole('button', { name: `${newFinding.cve_id} 패키지 2개 접기` }))
    expect(screen.queryByRole('table', { name: `${newFinding.cve_id} 패키지별 조치` })).not.toBeInTheDocument()
    await act(async () => { fireEvent.click(screen.getByLabelText('해결된 CVE 포함')) })
    expect(findings.getByText('패키지 3개')).toBeInTheDocument()
  ...
  it('2차 검증 결과를 배지와 요약으로 보여 주고 범위 선택으로 조치 대상만 본다', async () => {
    data.findings = [{ ...newFinding, tracker_status: 'pending', tracker_fix: '7.0.0-1014.14', host_relevance: 'UNLOADED_MODULE', kernel_files: ['drivers/gpu/drm/i915/i915_gem.c'] }]
    data['/api/analyses'] = [{ ...analysis, verification: { tracker: { pending: 900, needed: 1800, released: 5 }, host: { CORE: 40, LOADED_MODULE: 10, UNLOADED_MODULE: 2000, UNKNOWN: 30 }, checked_cves: 3100, fixed_already: 2, partial: false } }]
    await openSecurity()
    expect(screen.getByText(/배포판 추적기 3,100건 대조 · 수정 예정 900 · 업스트림만 수정 1800 · 이미 수정됨\(DB 지연\) 2/)).toBeInTheDocument()
    expect(screen.getByText(/핵심·로드됨 50 · 이 서버 미해당 추정 2000 · 확인 필요 30/)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '검사 20 2차 검증' })).toBeInTheDocument()
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: '검사 20 CVE 보기' })) })
    expect(screen.getByLabelText('표시 범위')).toHaveValue('actionable')
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'CVE별로 보기' })) })
    const findings = within(screen.getByRole('table', { name: '선택한 SBOM의 CVE' }))
    expect(findings.getByText('다음 패키지에서 수정 예정 7.0.0-1014.14')).toBeInTheDocument()
    expect(findings.getByText('미로드 드라이버 (해당 없음 추정)')).toBeInTheDocument()
    await act(async () => { fireEvent.change(screen.getByLabelText('표시 범위'), { target: { value: 'all' } }) })
    expect(fetch.mock.calls.some(([url]) => url.includes('relevance=all'))).toBe(true)
  })
  ...
  it('CVE를 100건씩 탐색하고 다른 SBOM을 선택하면 첫 페이지로 돌아간다', async () => {
    ...
    expect(screen.getByText('CVE 101개 · 101–101개 표시')).toBeInTheDocument()
    const pageRequest = fetch.mock.calls.find(([url]) => url === '/api/vulnerability-work/cves?sbom_id=2&status=OPEN&fix=ALL&kernel=false&relevance=actionable&limit=100&offset=100')
  ...
  it('SBOM을 선택하기 전에는 CVE를 가져오지 않고 선택을 바꾸면 느린 이전 응답을 폐기한다', async () => {
    const defaultFetch = fetch.getMockImplementation()
    let resolveOld
    fetch.mockImplementation((url, options) => url.includes('/vulnerability-work/cves?sbom_id=2') ? new Promise((resolve) => { resolveOld = resolve }) : defaultFetch(url, options))
    await openSecurity()
    ...
    fireEvent.click(screen.getByRole('button', { name: 'CVE별로 보기' }))
    const oldRequest = fetch.mock.calls.find(([url]) => url.includes('/vulnerability-work/cves?sbom_id=2'))
    ...
    await act(async () => { resolveOld(jsonResponse(cvePage([newFinding], oldRequest[0]))) })
  ...
  it('CVE 페이지 조회 실패를 빈 결과와 구별하고 다시 시도하며 화면 종료 시 조회를 취소한다', async () => {
    ...
    fetch.mockImplementation((url, options) => {
      if (url.startsWith('/api/vulnerability-work/cves?') && ++attempts === 1) return Promise.resolve({ ok: false, status: 503, json: async () => ({ detail: '잠시 후 다시 요청하세요.' }) })
      return defaultFetch(url, options)
    })
    ...
    expect(screen.getByText(newFinding.cve_id)).toBeInTheDocument()
    let resolvePending
    fetch.mockImplementation((url, options) => url.includes('/vulnerability-work/cves?sbom_id=1') ? new Promise((resolve) => { resolvePending = resolve }) : defaultFetch(url, options))
    fireEvent.change(screen.getByLabelText('확인할 SBOM'), { target: { value: '1' } })
    const pendingRequest = fetch.mock.calls.find(([url]) => url.includes('/vulnerability-work/cves?sbom_id=1'))
    view.unmount()
    expect(pendingRequest[1].signal.aborted).toBe(true)
    await act(async () => { resolvePending(jsonResponse(cvePage([oldFinding], pendingRequest[0]))) })
  })
...
describe('구성요소별 CVE 보기', () => {
  ...
  it('같은 구성요소의 CVE를 한 줄로 접어 두고 펼치면 그 아래에 나열한다', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      if (url.startsWith('/api/vulnerability-work/cves?')) return { ok: true, status: 200, json: async () => cvePage(findings, url) }
    ...
    expect(fetch.mock.calls.some(([url]) => url === '/api/vulnerability-work?sbom_id=2&component_id=1&status=OPEN&fix=ALL&kernel=true&relevance=actionable&limit=100&offset=0')).toBe(true)
...
describe('프로젝트와 대상의 결과 문맥 연결', () => {
  ...
  it('프로젝트에서 오래된 CVE 결과·SBOM 상세를 왕복하고 대상의 CVE 보기로 돌아온다', async () => {
    await openProject()
    ...
    await waitFor(() => expect(screen.getByLabelText('확인할 SBOM')).toHaveValue('101'))
    openHistoryView('검사 이력')
    fireEvent.click(await screen.findByRole('button', { name: '이 대상의 CVE 보기' }))
    await waitFor(() => expect(screen.getByLabelText('확인할 SBOM')).toHaveValue('101'))
    expect(fetch.mock.calls.some(([url]) => url === '/api/vulnerabilities')).toBe(false)
    expect(fetch.mock.calls.every(([, options]) => !options.method)).toBe(true)
  })
...
describe('서버 분석 전후 비교', () => {
  const base = { id: 10, sbom_id: 110, asset_id: 7, asset_tag: 'VM-001', asset_name: '시연 서버', scan_scope: 'ubuntu-dpkg-installed', imported_at: '2026-09-15T06:00:00Z', scanner: 'grype' }
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (url) => {
      ...
      if (url === '/api/analyses') return response([base, target, { ...base, id: 5 }, { ...base, id: 30, scan_scope: 'source-zip:orders' }, { ...base, id: 40, asset_id: 8, asset_tag: 'VM-002' }, { ...base, id: 50 }, { ...base, id: 60, asset_id: null }])
