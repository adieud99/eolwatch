    ...
    <section className="panel workspace-panel" aria-label="프로젝트 분석">
      ...
      {canEdit ? <>
        ...
        <form className="workspace-form" onSubmit={submit}><fieldset disabled={!!busy}><legend>{mode === 'zip' ? (sourceKind === 'git' ? '검사할 Git 저장소' : '검사할 프로젝트') : '검사할 서버와 경로'}</legend>
          <label>{mode === 'zip' ? '결과를 저장할 대상' : '검사할 서버'}<select required value={assetId} onChange={(event) => setAssetId(event.target.value)}><option value="">대상 선택</option>{assets.map((item) => <option key={item.id} value={item.id}>{item.asset_tag} · {item.name}</option>)}</select></label>
