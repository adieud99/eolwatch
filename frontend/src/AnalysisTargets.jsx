...
export default function AnalysisTargets({ mode: fixedMode, assets, canEdit, request, onJobQueued, initialAssetId, initialScope, initialProjectName, initialTargetPath }) {
  ...
  useEffect(() => {
    ...
    if (fixedMode !== 'zip' && initialScope && isExecutableScope(initialScope) && !initialScope.startsWith('source-zip:')) {
      ...
      setMode(fixedMode || 'zip'); setProject(initialScope && !isExecutableScope(initialScope) ? '' : initialProjectName || (initialScope?.startsWith('source-zip:') ? initialScope.slice('source-zip:'.length) : ''))
    }
  }, [initialAssetId, initialScope, initialProjectName, initialTargetPath, fixedMode])
