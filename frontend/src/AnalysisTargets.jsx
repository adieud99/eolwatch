    ...
    <section className="panel workspace-panel" aria-label="프로젝트 분석">
      ...
      {canEdit ? <>
        ...
        <form className="workspace-form" onSubmit={submit}><fieldset disabled={!!busy}><legend>{sourceKind === 'git' ? '검사할 Git 저장소' : '검사할 프로젝트'}</legend>
          <label>결과를 저장할 대상<div className="with-button"><select required value={assetId} onChange={(event) => setAssetId(event.target.value)}><option value="">대상 선택</option>{targets.map((item) => <option key={item.id} value={item.id}>{item.asset_tag} · {item.name}</option>)}</select>{!creating && <button type="button" className="table-button" onClick={() => { setCreating(true); setError('') }}>+ 새 대상</button>}</div></label>
