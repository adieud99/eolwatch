...
const profiles = { 'ssh-project-directory': '서버의 프로젝트 폴더', 'ssh-python-environment': '서버의 Python 가상환경', 'ubuntu-dpkg-installed': 'OS 설치 패키지', 'demo-python-venv': '데모 앱 (Python)' }
...
export default function AnalysisTargets({ mode: fixedMode, assets, canEdit, request, onJobQueued, onAssetCreated, initialAssetId, initialScope, initialProjectName, initialTargetPath, initialGitUrl, initialGitRef }) {
  ...
  const [creating, setCreating] = useState(false)       // inline '새 대상' form for source scans
    ...
    try {
      if (mode === 'zip' && sourceKind === 'git') {
        ...
        if (!controller.signal.aborted) {
          if (scheduled) { setMessage('정기 검사를 등록했습니다.'); await loadSchedules() }
  ...
  return <>
    <section className="panel workspace-panel" aria-label="프로젝트 분석">
      {fixedMode === 'zip' ? <><span className="eyebrow">SOURCE SCAN</span><h2>소스 검사</h2><p className="workspace-help">소스 ZIP이나 의존성 파일을 올리거나 Git 저장소 주소를 입력하면 의존성 목록을 뽑아 알려진 취약점과 대조합니다. 결과는 아래에서 고른 대상 아래에 프로젝트 이름으로 쌓입니다.</p></> : fixedMode === 'ssh' ? <><span className="eyebrow">SERVER APP SCAN</span><h2>서버 앱 경로 검사</h2><p className="workspace-help">등록된 서버의 앱 폴더나 Python 가상환경 경로를 지정해 검사합니다. 정기 검사도 여기서 등록합니다.</p></> : <><span className="eyebrow">PROJECT ANALYSIS</span><h2>프로젝트 분석</h2><p className="workspace-help">프로젝트 소스 ZIP을 올리거나 등록된 서버의 앱 경로를 지정하세요. 검사 결과는 선택한 대상에 연결됩니다.</p></>}
      ...
      {canEdit ? <>
        ...
        <form className="workspace-form" onSubmit={submit}><fieldset disabled={!!busy}><legend>{mode === 'zip' ? (sourceKind === 'git' ? '검사할 Git 저장소' : '검사할 프로젝트') : '검사할 서버와 경로'}</legend>
          <label>{mode === 'zip' ? '결과를 저장할 대상' : '검사할 서버'}<select required value={assetId} onChange={(event) => setAssetId(event.target.value)}><option value="">대상 선택</option>{[...assets, ...created.filter((item) => !assets.some((known) => known.id === item.id))].map((item) => <option key={item.id} value={item.id}>{item.asset_tag} · {item.name}</option>)}</select>{mode === 'zip' && canEdit && !creating && <button type="button" className="table-button" onClick={() => { setCreating(true); setError('') }}>+ 새 대상</button>}</label>
          {mode === 'zip' && creating && <fieldset className="new-target"><legend>새 대상 (검사 결과를 모아 둘 이름)</legend>
          ...
          : mode === 'zip' ? <><label>프로젝트 이름<input required maxLength={80} value={project} onChange={(event) => setProject(event.target.value)} placeholder="예: 주문 API" /></label><label className="workspace-full">소스 ZIP 또는 의존성 파일<input ref={fileInput} type="file" required accept={MANIFEST_ACCEPT} onChange={(event) => setFile(event.target.files?.[0] || null)} /></label><p className="workspace-help workspace-full">최대 500 MiB. 잠금 파일과 패키지 명세가 포함된 소스 ZIP, 또는 requirements.txt·package-lock.json·pom.xml·go.sum 같은 의존성 파일 하나를 선택하세요. 같은 대상·프로젝트 이름으로 다음 버전을 올리면 전후 비교할 수 있습니다. 패키지를 설치하거나 프로그램을 실행하지 않으며, 포함된 메타데이터에서 구성요소를 식별합니다.</p></> : <><label>검사 방식<select value={profile} onChange={(event) => setProfile(event.target.value)}>{Object.entries(profiles).map(([value, name]) => <option key={value} value={value}>{name}</option>)}</select></label>{pathRequired && <label className="workspace-full">서버 앱 절대 경로<input required value={path} onChange={(event) => setPath(event.target.value)} placeholder={profile === 'ssh-python-environment' ? '/opt/my-app/.venv' : '/opt/my-app'} /></label>}{asset && !sshReady && <p className="form-error">이 서버에는 IP·SSH 계정·검사 대상 설정이 필요합니다. 인프라 검사의 서버 수정에서 설정하세요.</p>}<label>정기 검사 주기 (분)<input type="number" min={5} max={525600} step={1} value={interval} onChange={(event) => setInterval(event.target.value)} /></label><p className="workspace-help">프로젝트 폴더는 잠금 파일과 패키지 명세를, 가상환경은 설치된 패키지 정보를 읽습니다. SSH 계정이 읽을 수 있는 경로여야 합니다.</p></>}
          {error && <p className="form-error" role="alert">{error}</p>}<div className="action-row workspace-full"><button className="primary" disabled={!!busy || !assetId || (mode === 'ssh' && !sshReady)}>{busy === 'analysis' ? '검사 요청 중…' : mode === 'zip' && sourceKind === 'git' ? '검사 시작' : '검사 시작'}</button>{mode === 'ssh' && <button type="button" className="secondary" disabled={!!busy || !sshReady} onClick={(event) => submit(event, true)}>{busy === 'schedule' ? '등록 중…' : '정기 검사 등록'}</button>}</div>
        </fieldset></form>
